...
register(
        id='DuckieTownEnv-v0',
        entry_point='duckbot_lane_following:DuckieTownEnv',
        timestep_limit=timestep_limit_per_episode,
    )

class DuckieTownEnv(duckietown_env.DuckieTownEnv):
    def __init__(self):
        """
        ...
        number_actions = rospy.get_param('/duckbot/n_actions')
        ...
        super(DuckieTownEnv, self).__init__()
    ...
    def discretize_observation(self, image_data,  image2_data):
        ...
        self.detection_results = self.line_follow.process_data(observation_image=self.camera_rgb_image_raw,
                                                            detection_image=self.camera2_rgb_image_raw,
                                                            rows_to_watch=self.look_ahead_distance,
                                                            show_window=True)
        
        
        self.cur_offset = float("%.4f" % self.detection_results[1]) #0.5  and 1.52
        
        if ((self.cur_offset < self.dSafe) or (self.cur_offset > self.dMax )):
                self._episode_done = True
        #try:
            
            #self.truncated_and_scaled_offset = float("%.2f" % float(self.observation_mapper(cur_offset)))
            #discretized_ranges.append(self.truncated_and_scaled_offset)
        #except ValueError:
         #   pass
        
        return self.detection_results[2] #return the image converted to numpy array as observation data
